...
router.get("/secret/:userId", secured, isAuth, (req, res) => {
	res.json({ user: req.profile }); //the 'user' was stored in the req obj wt profile as the 'key' in authMiddleware.js
});
...
/*Anytime there is a 'userId' param in the route we run the userByid middleware 
which make d user info available in the 'req' obj thru the 'req.profile'
